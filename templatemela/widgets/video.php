...
function wp_widget_video($args, $widget_args = 1) {
	...
	switch ($type)
	{
		...
		case 'flv':
			...
			$videoWidgetFolder = get_template_directory_uri() .'/wp-content/plugins/video-widget/';
...
function wp_widget_video_register() {
	if ( !$options = get_option('widget_video') )
		$options = array();
	$widget_ops = array('classname' => 'widget_video', 'description' => __('Adds YouTube/Dailymotion/Google... video','templatemela'));
	$control_ops = array('width' => 460, 'height' => 350, 'id_base' => 'video');
	$name = __('TM - Video Widget','templatemela');
